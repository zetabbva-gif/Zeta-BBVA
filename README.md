...
| `assets/pablo.png`           | Foto del Hero (columna derecha)            | Se muestra tal cual, sin recortar ni procesar — fondo blanco propio de la imagen (la columna del hero también es blanca, `.hero-media` en `css/hero.css`) |
...
> **Nota sobre `assets/pablo.png`:** el hero ya no usa `mix-blend-mode`,
> `filter` ni overlays sobre la imagen — se muestra tal cual, con su
> fondo blanco original, en la columna derecha del hero (`.hero-media`
> en `css/hero.css`, que también es blanca). No hace falta recortar el
> fondo ni prepararla de ninguna forma especial: cualquier imagen que
> pongas ahí (incluido el texto que lleve encima) se verá completa.
